import React, { useState } from 'react';
import PropTypes from 'prop-types';
import Button from './Button';

const Alert = ({
  children,
  title,
  variant = 'info',
  dismissible = false,
  icon = true,
  bordered = false,
  rounded = 'md',
  onClose,
  className = '',
  ...props
}) => {
  const [visible, setVisible] = useState(true);

  // Base classes
  const baseClasses = 'p-4';
  
  // Variant classes
  const variantClasses = {
    info: 'bg-blue-50 text-blue-800 dark:bg-blue-900 dark:text-blue-200',
    success: 'bg-green-50 text-green-800 dark:bg-green-900 dark:text-green-200',
    warning: 'bg-yellow-50 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200',
    error: 'bg-red-50 text-red-800 dark:bg-red-900 dark:text-red-200',
    neutral: 'bg-gray-50 text-gray-800 dark:bg-gray-700 dark:text-gray-200'
  };
  
  // Border classes
  const borderClasses = {
    info: 'border border-blue-200 dark:border-blue-700',
    success: 'border border-green-200 dark:border-green-700',
    warning: 'border border-yellow-200 dark:border-yellow-700',
    error: 'border border-red-200 dark:border-red-700',
    neutral: 'border border-gray-200 dark:border-gray-600'
  };
  
  // Rounded classes
  const roundedClasses = {
    none: 'rounded-none',
    sm: 'rounded-sm',
    md: 'rounded-md',
    lg: 'rounded-lg'
  };
  
  // Icons for each variant
  const icons = {
    info: (
      <svg className="h-5 w-5 text-blue-400 dark:text-blue-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
      </svg>
    ),
    success: (
      <svg className="h-5 w-5 text-green-400 dark:text-green-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
      </svg>
    ),
    warning: (
      <svg className="h-5 w-5 text-yellow-400 dark:text-yellow-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
      </svg>
    ),
    error: (
      <svg className="h-5 w-5 text-red-400 dark:text-red-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" />
      </svg>
    ),
    neutral: (
      <svg className="h-5 w-5 text-gray-400 dark:text-gray-300" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
        <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
      </svg>
    )
  };

  const handleClose = () => {
    setVisible(false);
    if (onClose) onClose();
  };

  if (!visible) return null;

  return (
    <div
      className={`
        ${baseClasses}
        ${variantClasses[variant]}
        ${bordered ? borderClasses[variant] : ''}
        ${roundedClasses[rounded]}
        ${className}
      `}
      role="alert"
      {...props}
    >
      <div className="flex">
        {icon && (
          <div className="flex-shrink-0 mr-3">
            {icons[variant]}
          </div>
        )}
        <div className="flex-1">
          {title && (
            <h3 className="text-sm font-medium mb-1">
              {title}
            </h3>
          )}
          <div className="text-sm">
            {children}
          </div>
        </div>
        {dismissible && (
          <div className="ml-auto pl-3">
            <div className="-mx-1.5 -my-1.5">
              <Button
                variant="ghost"
                onClick={handleClose}
                className={`inline-flex rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  variant === 'info' ? 'bg-blue-50 text-blue-500 hover:bg-blue-100 focus:ring-blue-600 focus:ring-offset-blue-50 dark:bg-blue-900 dark:text-blue-400 dark:hover:bg-blue-800 dark:focus:ring-blue-500 dark:focus:ring-offset-blue-900' :
                  variant === 'success' ? 'bg-green-50 text-green-500 hover:bg-green-100 focus:ring-green-600 focus:ring-offset-green-50 dark:bg-green-900 dark:text-green-400 dark:hover:bg-green-800 dark:focus:ring-green-500 dark:focus:ring-offset-green-900' :
                  variant === 'warning' ? 'bg-yellow-50 text-yellow-500 hover:bg-yellow-100 focus:ring-yellow-600 focus:ring-offset-yellow-50 dark:bg-yellow-900 dark:text-yellow-400 dark:hover:bg-yellow-800 dark:focus:ring-yellow-500 dark:focus:ring-offset-yellow-900' :
                  variant === 'error' ? 'bg-red-50 text-red-500 hover:bg-red-100 focus:ring-red-600 focus:ring-offset-red-50 dark:bg-red-900 dark:text-red-400 dark:hover:bg-red-800 dark:focus:ring-red-500 dark:focus:ring-offset-red-900' :
                  'bg-gray-50 text-gray-500 hover:bg-gray-100 focus:ring-gray-600 focus:ring-offset-gray-50 dark:bg-gray-700 dark:text-gray-400 dark:hover:bg-gray-600 dark:focus:ring-gray-500 dark:focus:ring-offset-gray-700'
                }`}
                icon={
                  <svg className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                    <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                  </svg>
                }
              >
                <span className="sr-only">Dismiss</span>
              </Button>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

// Toast notification that auto-dismisses
export const Toast = ({
  message,
  title,
  variant = 'info',
  duration = 5000,
  position = 'top-right',
  onClose,
  className = '',
  ...props
}) => {
  const [visible, setVisible] = useState(true);

  // Position classes
  const positionClasses = {
    'top-right': 'fixed top-4 right-4',
    'top-left': 'fixed top-4 left-4',
    'bottom-right': 'fixed bottom-4 right-4',
    'bottom-left': 'fixed bottom-4 left-4',
    'top-center': 'fixed top-4 left-1/2 transform -translate-x-1/2',
    'bottom-center': 'fixed bottom-4 left-1/2 transform -translate-x-1/2'
  };

  React.useEffect(() => {
    if (duration) {
      const timer = setTimeout(() => {
        setVisible(false);
        if (onClose) onClose();
      }, duration);
      
      return () => clearTimeout(timer);
    }
  }, [duration, onClose]);

  const handleClose = () => {
    setVisible(false);
    if (onClose) onClose();
  };

  if (!visible) return null;

  return (
    <div 
      className={`
        max-w-sm w-full shadow-lg rounded-lg pointer-events-auto
        ring-1 ring-black ring-opacity-5 overflow-hidden z-50
        ${positionClasses[position]}
        ${className}
      `}
      {...props}
    >
      <Alert
        variant={variant}
        title={title}
        dismissible
        onClose={handleClose}
        className="animate-slide-in"
      >
        {message}
      </Alert>
    </div>
  );
};

Alert.propTypes = {
  children: PropTypes.node.isRequired,
  title: PropTypes.string,
  variant: PropTypes.oneOf(['info', 'success', 'warning', 'error', 'neutral']),
  dismissible: PropTypes.bool,
  icon: PropTypes.bool,
  bordered: PropTypes.bool,
  rounded: PropTypes.oneOf(['none', 'sm', 'md', 'lg']),
  onClose: PropTypes.func,
  className: PropTypes.string
};

Toast.propTypes = {
  message: PropTypes.string.isRequired,
  title: PropTypes.string,
  variant: PropTypes.oneOf(['info', 'success', 'warning', 'error', 'neutral']),
  duration: PropTypes.number,
  position: PropTypes.oneOf(['top-right', 'top-left', 'bottom-right', 'bottom-left', 'top-center', 'bottom-center']),
  onClose: PropTypes.func,
  className: PropTypes.string
};

export default Alert;